...
22. **Docstring stale `orchestrator-v3.ts:2` dice "23 agents".** Post-eliminación de pages-routing el conteo real es 22 (`AGENT_NAMES_V3`, ver `orchestrator-v3.test.ts` "includes all 22 agents"). Cosmético, ajeno a B-w4-9, no tocado para evitar scope creep. Corregir en el próximo pase de cleanup junto con deuda #21. Descubierto durante B-w4-9 (Commit A).

23. **Audit boundary validators en todos los v3 agents.** Tras B-w4-11/12 se agregó `validators` al slot `visual-adapter` (full-yoga-regen-v3.ts). Pendiente auditar que cada agent v3 con schema tenga su validator wired al slot: `layout-architect` ya tiene `test-id-contract.json`, `ui-components` tiene `components-catalog.json`; **sin auditar**: `bootstrap-devops`, `ux-ui-designer`, `brand-identity`, y el resto de v2-reused con schema v3 (`api-backend`, `frontend-architect`, `domain-modeler`, `persistence`, etc.). Sin validator = falso-verde ante drift LLM (clase B-w4-12). Pase futuro de hardening. Origen: B-w4-11/12.
