...
23. **Audit boundary validators en todos los v3 agents.** Slots con validator wired: `visual-adapter` (`page-adaptations.json`, B-w4-12), `forms-validations` (`forms-validations.json` v3, ✅ **B-w4-13**), `layout-architect` (`test-id-contract.json`), `ui-components` (`components-catalog.json`), ✅ **`seeds-fixtures`** (`seeds-fixtures.json`, schema desde run-14 + **prompt v3 vinculante post-B-w5-1**), ✅ **`tests-writer`** (`tests-writer.json`, ídem + prompt v3). **wave-5 CERRADO + endurecido (B-w5-1: schema+validator+prompt skeleton, contrato bidireccional B-w4-13).** **Sin auditar (pendiente)**: `bootstrap-devops`, `ux-ui-designer`, `brand-identity`, `qa-reviewer` (wave-6, `qa-report.json` — slot YA wirado este pase; schema/validator TBD post-F3-run-15, shape-first), y el resto de v2-reused con schema v3 (`api-backend`, `frontend-architect`, `domain-modeler`, `persistence`, etc.). Sin validator = falso-verde ante drift LLM (clase B-w4-12). Pase futuro de hardening. Origen: B-w4-11/12; slot forms-validations cerrado en B-w4-13; **slots wave-5 cerrados con schemas shape-first desde F3-run-14 (este pase)**.
