/**
 * Día 5 — Atelier v2 tutorias end-to-end real regeneration.
 *
 * Sibling of `full-yoga-regen.ts` — same orchestration plumbing, points at
 * `fixtures/tutorias-prd.json`. The tutorias PRD already comes in v2-discovery
 * shape (objective + domain + roles + entities-as-objects + useCases-as-
 * objects), so the conversion mostly does role translation
 * (`tutor` → `teacher`, `estudiante` → `student`) + normalization rather
 * than the heavier inference path that yoga's flat PRD needed.
 *
 * Pipeline:
 *   1. Clone skeleton-v2 → out/tutorias-regen-v2-<ts>/
 *   2. Convert fixtures/tutorias-prd.json → discovery v2 schema, inject as
 *      .atelier/discovery.json (Discovery agent is SKIPPED).
 *   3-7. (Identical to yoga) — orchestrator-v2 with 6 waves, --real / --dry-run,
 *        fix loop, pnpm qa, FINAL tag.
 *
 * Usage:
 *   tsx scripts/full-tutorias-regen.ts --real        # full LLM regen
 *   tsx scripts/full-tutorias-regen.ts --dry-run     # synthetic regen (fast)
 *   tsx scripts/full-tutorias-regen.ts --resume <workDir|latest>
 */
...
const PRD_FIXTURE = join(REPO_ROOT, "fixtures", "tutorias-prd.json");
const WORKDIR_PREFIX = "tutorias-regen-v2-";
 ...
 * `"latest"` = auto-pick most recent out/tutorias-regen-v2-* directory. */
...
// Tutorias PRD already comes in v2 shape (fields as objects, useCases as
// objects, top-level `domain` + `designVibe`). The yoga PRD was flat.
// `convertPrdToDiscovery` accepts both shapes via the `PrdFixture` union.
...
interface PrdFixture {
  ...
  readonly designVibe?: string;
  readonly roles: ReadonlyArray<string>;
  readonly entities: ReadonlyArray<{
    readonly name: string;
    readonly fields: ReadonlyArray<FieldEntry>;
    readonly notes?: string;
    readonly businessRules?: ReadonlyArray<string>;
  }>;
  readonly useCases: ReadonlyArray<UseCaseEntry>;
  readonly notes?: ReadonlyArray<string>;
  readonly specialRequirements?: ReadonlyArray<string>;
}
...
function inferFieldType(fieldName: string): string {
  ...
  if (n.includes("role") || n.includes("level") || n.includes("tier") || n.includes("status") || n.includes("format"))
    return "enum";
  if (n.endsWith("at") || n.includes("date") || n.includes("from") || n.includes("until") || n.includes("scheduled"))
    return "date";
  if (
    n.includes("duration") ||
    n.includes("count") ||
    ...
    return "number";
  if (n.startsWith("is") || n.startsWith("has") || n === "active" || n === "verified") return "boolean";
  if (n.includes("url")) return "string";
  return "string";
}
...
function normalizeField(entry: FieldEntry): { name: string; type: string; required: boolean; notes?: string; references?: string; values?: string[] } {
  if (typeof entry === "string") {
    return { name: entry, type: inferFieldType(entry), required: isRequiredField(entry) };
  }
  const out: { name: string; type: string; required: boolean; notes?: string; references?: string; values?: string[] } = {
    name: entry.name,
    type: entry.type ?? inferFieldType(entry.name),
    required: entry.required ?? isRequiredField(entry.name),
  };
  if (entry.notes) out.notes = entry.notes;
  if (entry.references) out.references = entry.references;
  if (entry.values) out.values = [...entry.values];
  return out;
}

function ensureCoreFields(
  fields: ReadonlyArray<FieldEntry>,
): Array<{ name: string; type: string; required: boolean; notes?: string; references?: string; values?: string[] }> {
  const normalized = fields.map(normalizeField);
  const seen = new Set(normalized.map((f) => f.name.toLowerCase()));
  const out: Array<{ name: string; type: string; required: boolean; notes?: string; references?: string; values?: string[] }> = [];
  // Always prepend id + slug if missing — discovery contract demands ≥3 fields
  // and downstream agents rely on these existing.
  if (!seen.has("id")) out.push({ name: "id", type: "string", required: true });
  if (!seen.has("slug")) out.push({ name: "slug", type: "string", required: true });
  out.push(...normalized);
  return out;
}

function parseUseCaseString(raw: string): { actor: string; action: string; constraints?: string[] } {
...
function normalizeUseCase(entry: UseCaseEntry): { actor: string; action: string; constraints?: string[] } {
  if (typeof entry === "string") return parseUseCaseString(entry);
  const actor = USECASE_ACTORS[entry.actor.toLowerCase()] ?? entry.actor.toLowerCase();
  const result: { actor: string; action: string; constraints?: string[] } = {
    actor,
    action: entry.action,
  };
  if (entry.constraints && entry.constraints.length > 0) {
    result.constraints = [...entry.constraints];
  }
  return result;
}
...
function convertPrdToDiscovery(prd: PrdFixture): Discovery {
  const discovery: Discovery = {
    ...
    domain: prd.domain ?? "tutorias",
    ...(prd.designVibe && ["Linear", "Stripe", "Notion", "Vercel", "Calm"].includes(prd.designVibe)
      ? { designVibe: prd.designVibe as Discovery["designVibe"] }
      : {}),
    roles: prd.roles.map((r) => ROLE_TRANSLATIONS[r.toLowerCase()] ?? r.toLowerCase()),
    entities: prd.entities.map((e) => {
      const explicit = e.businessRules ? [...e.businessRules] : [];
      const fromNotes = e.notes
        ? e.notes
            .split(/\.\s+/)
            .map((s) => s.trim().replace(/\.$/, ""))
            .filter((s) => s.length > 0)
        : [];
      const businessRules = [...explicit, ...fromNotes];
      return {
        name: e.name,
    ...
    useCases: prd.useCases.map(normalizeUseCase),
    ...(prd.specialRequirements && prd.specialRequirements.length > 0
      ? { specialRequirements: [...prd.specialRequirements] }
      : prd.notes && prd.notes.length > 0
        ? { specialRequirements: [...prd.notes] }
        : {}),
  };
  return discovery;
}
...
async function makeLogger(logFile: string): Promise<(stage: string, msg: string) => Promise<void>> {
  // touch the file
  await writeFile(logFile, `# tutorias-regen v2 — started ${ts()}\n\n`, "utf-8");
...
async function resolveResumeWorkDir(arg: string): Promise<string> {
  if (arg === "latest") {
    ...
    const matches = entries
      .filter((e) => e.startsWith(WORKDIR_PREFIX))
      .sort()
      .reverse();
    const newest = matches[0];
    if (!newest) throw new Error(`no out/${WORKDIR_PREFIX}* directories found for --resume latest`);
...
async function main(): Promise<number> {
  ...
  if (resumeActive) {
    ...
    workDir = resolve(REPO_ROOT, "out", `${WORKDIR_PREFIX}${tsTag}`);
    await mkdir(workDir, { recursive: true });
  }
