/**
 * v2 cumulative dry-run E2E (Wave 1 → 4). Validates that all 13 artifacts
 * produced by the 12 wave-1-through-4 agents pass their Zod schemas, that
 * the synthetic workDir compiles tsc, and that the prisma schema parses.
 *
 * 13 artifacts = 5 (wave 1+2) + 3 (wave 3) + 5 (wave 4):
 *   wave 1: design-system, screens-map
...
async function setupWorkDir(): Promise<string> {
  const workDir = resolve(REPO_ROOT, "out", "test-wave1234-workdir");
...
async function dryRun(workDir: string): Promise<void> {
  ...
  // Synthesize a representative slice of the cumulative output.
  log("dry-run", "writing synthetic wave 1→4 sample (entity + repo + service + authz + controller + react component + page)");

  // prisma schema (wave 2)
  await writeFile(
  ...
  // wave 3: AuthorizationService + AuthService stubs
  await writeFile(
  ...
  await writeFile(
    ...
    [
      ...
      `    return true; // stub for typecheck — real impl uses CASL`,
      `  }`,
  ...
  // wave 4: GlobalExceptionHandler + thin controller + minimal client component + page
  await writeFile(
  ...
  // client side: a thin component + a page composing it
  ...
  // page composing the component (uses @client alias) — mkdir THEN writeFile
  ...
  log("dry-run", "running pnpm typecheck on cumulative wave 1-4 slice");
  ...
  log("dry-run", "  ✓ pnpm typecheck green over wave 1+2+3+4 synthetic slice");
  ...
  // Bonus: verify ESLint doesn't choke on the synthetic slice
...
async function main(): Promise<number> {
  ...
  log("init", "MODE: --dry-run cumulative wave 1→4");
  ...
  log("done", `✅ wave 1→4 cumulative dry-run green in ${((Date.now() - startedAt) / 1000).toFixed(1)}s`);
  log("done", `   workDir preserved at: ${workDir}`);
  log("done", `   13 artifacts validated, tsc + lint + prisma all green`);
  return 0;
}
